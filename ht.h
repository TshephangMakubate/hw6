...
{
  double currentLoad = (double)numItems_ / (double)table_.size() ;
  if (currentLoad >= alpha_){
    resize();
  }
...
{
  if(mIndex_ >= 27){
    throw std::logic_error("Max table size reached") ;
  }
  mIndex_++ ;
  ...
  for(size_t i = 0; i < oldTable.size() ; i++){
    if(oldTable[i] != nullptr){
      if(!oldTable[i]->deleted){
        ...
        if(idx == npos) throw std::logic_error("Resize failed") ;
        table_[idx] = oldTable[i] ;
        numItems_++ ;
      }
      else{
        delete oldTable[i] ;
